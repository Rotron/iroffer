...
#define SIZESTR_SIZE 5
    ...
    if (num >= 1024 * 1024 * 1024 * 1000ULL) {
        /* >1000GB */
        snprintf(str, SIZESTR_SIZE, "%2.1fT",
                 (((float)num) / (1024.0 * 1024.0 * 1024.0 * 1024.0)));
    } else if (num >= 1024 * 1024 * 1024 * 10ULL) {
        /* >10GB */
        snprintf(str, SIZESTR_SIZE, "%3.0fG",
                 (((float)num) / (1024.0 * 1024.0 * 1024.0)));
    } else if (num >= 1024 * 1024 * 1000) {
        /* >1000MB */
        snprintf(str, SIZESTR_SIZE, "%2.1fG",
                 (((float)num) / (1024.0 * 1024.0 * 1024.0)));
    } else if (num >= 1024 * 1024 * 10) {
        /* >10MB */
        snprintf(str, SIZESTR_SIZE, spaces ? "%3.0fM" : "%.0fM",
                 (((float)num) / (1024.0 * 1024.0)));
    } else if (num >= 1024 * 1000) {
        /* >1000KB */
        snprintf(str, SIZESTR_SIZE, "%2.1fM",
                 (((float)num) / (1024.0 * 1024.0)));
    } else if (num >= 1024) {
        /* >1KB */
        snprintf(str, SIZESTR_SIZE, spaces ? "%3.0fK" : "%.0fK",
                 (((float)num) / 1024.0));
    } else if (num >= 1) {
        /* <1KB */
        snprintf(str, SIZESTR_SIZE, spaces ? " <1K" : "<1K");
    } else {
        snprintf(str, SIZESTR_SIZE, spaces ? "   0" : "0");
    }
...
void getos(void) {
    ...
    printf("** You Are Running %s %s on a %s", u1.sysname, u1.release,
           u1.machine);
    mylog(CALLTYPE_NORMAL, "You Are Running %s %s on a %s", u1.sysname,
          u1.release, u1.machine);

    gdata.osstring = mymalloc(strlen(u1.sysname) + strlen(u1.release) + 2);

    sprintf(gdata.osstring, "%s %s", u1.sysname, u1.release);
    ...
    defined(_OS_BSDI) || defined(_OS_BSD_OS)
    if (strcmp(u1.sysname, "FreeBSD") && strcmp(u1.sysname, "OpenBSD") &&
        strcmp(u1.sysname, "BSD/OS") && strcmp(u1.sysname, "NetBSD"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for *BSD but not running *BSD?!?");
    printf(", Good\n");

#elif defined(_OS_SunOS)
    if (strcmp(u1.sysname, "SunOS"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for Solaris but not running Solaris?!?");
    printf(", Good\n");

#elif defined(_OS_HPUX)
    if (strcmp(u1.sysname, "HP-UX"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for HP-UX but not running HP-UX?!?");
    printf(", Good\n");

#elif defined(_OS_IRIX)
    if (strcmp(u1.sysname, "IRIX"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for IRIX but not running IRIX?!?");
    printf(", Good\n");

#elif defined(_OS_IRIX64)
    if (strcmp(u1.sysname, "IRIX64"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for IRIX64 but not running IRIX64?!?");
    printf(", Good\n");

#elif defined(_OS_OSF1)
    if (strcmp(u1.sysname, "OSF1"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for OSF1 but not running OSF1?!?");
    printf(", Good\n");

#elif defined(_OS_Rhapsody)
    if (strcmp(u1.sysname, "Rhapsody"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for Rhapsody but not running Rhapsody?!?");
    printf(", Good\n");

#elif defined(_OS_Darwin)
    if (strcmp(u1.sysname, "Darwin"))
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Configured for Darwin but not running Darwin?!?");
    printf(", Good\n");
...
void outerror(outerror_type_e type, const char* format, ...) {
    ...
    if (type == OUTERROR_TYPE_CRASH) {
        ...
        ioutput(CALLTYPE_NORMAL, ioutput_options, COLOR_RED, "WARNING: %s",
                tempstr);
    }
}
...
char* getuptime(char* str, int type, time_t fromwhen, int len) {
    ...
    if (type) {
        llen = snprintf(str, len, "%dD %dH %dM", days, hours, mins);
    } else {
        llen =
            snprintf(str, len, "%d Days %d Hrs and %d Min", days, hours, mins);
    }
    ...
    if (gdata.logfd == FD_UNUSED) {
        ...
        if (gdata.logfd < 0) {
            outerror(OUTERROR_TYPE_WARN_LOUD | OUTERROR_TYPE_NOLOG,
                     "Cant Access Log File '%s': %s", gdata.logfile,
                     strerror(errno));
...
void vioutput(calltype_e type, int dest, unsigned int color_flags,
    ...
    if ((len < 0) || (len >= maxtextlength)) {
        snprintf(tempstr, maxtextlength - 1,
                 "IOUTPUT-INT: Output too large, ignoring!");
    }
    ...
    if (!gdata.background && (dest & OUT_S)) {
        ...
        if ((type == CALLTYPE_NORMAL) || (type == CALLTYPE_MULTI_FIRST)) {
            if (!gdata.nocolor && (color_flags != COLOR_NO_COLOR)) {
                tostdout("\x1b[%d;%dm", (color_flags & COLOR_BOLD) ? 1 : 0,
    ...
    if (dest & OUT_D) {
        for (chat = irlist_get_head(&gdata.dccchats); chat;
            ...
            if (chat->status == DCCCHAT_CONNECTED) {
                if ((type == CALLTYPE_NORMAL) ||
                    (type == CALLTYPE_MULTI_FIRST)) {
                    writedccchat(chat, 0, "--> ");
                }
...
void vprivmsg_slow(const char* nick, const char* format, va_list ap) {
    ...
    if ((len < 0) || (len >= maxtextlength)) {
        outerror(OUTERROR_TYPE_WARN,
                 "PRVMSG-SLOW: Output too large, ignoring!");
        return;
    }
...
void vprivmsg_fast(const char* nick, const char* format, va_list ap) {
    ...
    if ((len < 0) || (len >= maxtextlength)) {
        outerror(OUTERROR_TYPE_WARN,
                 "PRVMSG-FAST: Output too large, ignoring!");
        return;
    }
...
void vnotice_slow(const char* nick, const char* format, va_list ap) {
    ...
    if ((len < 0) || (len >= maxtextlength)) {
        outerror(OUTERROR_TYPE_WARN,
                 "NOTICE-SLOW: Output too large, ignoring!");
        return;
    }
...
void vnotice_fast(const char* nick, const char* format, va_list ap) {
    ...
    if ((len < 0) || (len >= maxtextlength)) {
        outerror(OUTERROR_TYPE_WARN,
                 "NOTICE-FAST: Output too large, ignoring!");
        return;
    }
    ...
    for (cc = 0; cc < MEMINFOHASHSIZE; cc++) {
        for (dd = 0; dd < gdata.meminfo_depth; dd++) {
            if (gdata.meminfo[(cc * (gdata.meminfo_depth)) + dd].ptr) {
                ...
                start =
                    alloc_hash(
                        gdata.meminfo[(cc * (gdata.meminfo_depth)) + dd].ptr) *
                ...
                newmeminfo[i] =
                    gdata.meminfo[(cc * (gdata.meminfo_depth)) + dd];
            }
        }
    }
...
void mydelete2(void* t) {
    ...
    if (i == (MEMINFOHASHSIZE * gdata.meminfo_depth)) {
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Pointer 0x%8.8lX not found in meminfo database while trying "
                 "to free!!",
                 (long)t);
        ...
        for (i = 0; i < (12 * 12); i += 12) {
            outerror(OUTERROR_TYPE_WARN_LOUD,
                     ...
                     "%2.2X %2.2X %2.2X = \"%c%c%c%c%c%c%c%c%c%c%c%c\"",
                     ut[i + 0], ut[i + 1], ut[i + 2], ut[i + 3], ut[i + 4],
                     ut[i + 5], ut[i + 6], ut[i + 7], ut[i + 8], ut[i + 9],
                     ut[i + 10], ut[i + 11], onlyprintable(ut[i + 0]),
                     onlyprintable(ut[i + 1]), onlyprintable(ut[i + 2]),
                     onlyprintable(ut[i + 3]), onlyprintable(ut[i + 4]),
                     onlyprintable(ut[i + 5]), onlyprintable(ut[i + 6]),
                     onlyprintable(ut[i + 7]), onlyprintable(ut[i + 8]),
                     onlyprintable(ut[i + 9]), onlyprintable(ut[i + 10]),
                     onlyprintable(ut[i + 11]));
        }
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Aborting Program! (core file should be generated)");
...
void dumpcontext(void) {
    ...
    for (i = 0; i < MAXCONTEXTS; i++) {
        ...
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_NO_COLOR,
                "Trace %3i  %-20s %-16s:%5i  %lu.%06lu", i - MAXCONTEXTS + 1,
                c->func ? c->func : "UNKNOWN", c->file ? c->file : "UNKNOWN",
...
#define gdata_print_number_array(format, name)                                 \
    {                                                                          \
        if (gdata.name[ii]) {                                                  \
            ioutput(gdata_common, "GDATA * " #name "[%d]: " format, ii,        \
                    gdata.name[ii]);                                           \
        }                                                                      \
    }
...
#define gdata_print_string_array(name)                                         \
    {                                                                          \
        if (gdata.name[ii]) {                                                  \
            ioutput(gdata_common, "GDATA * " #name "[%d]: %s", ii,             \
                    gdata_string(gdata.name[ii]));                             \
        }                                                                      \
    }

#define gdata_print_number_array_item(format, name, item)                      \
    {                                                                          \
        if (gdata.name[ii].item) {                                             \
            ioutput(gdata_common, "GDATA * " #name "[%d]: " #item "=" format,  \
                    ii, gdata.name[ii].item);                                  \
        }                                                                      \
    }
...
#define gdata_print_string_array_item(name, item)                              \
    {                                                                          \
        if (gdata.name[ii].item) {                                             \
            ioutput(gdata_common, "GDATA * " #name "[%d]: " #item "=%s", ii,   \
                    gdata_string(gdata.name[ii].item));                        \
        }                                                                      \
    }
...
#define gdata_irlist_iter_start(name, type)                                    \
    {                                                                          \
        type* iter;                                                            \
        ioutput(gdata_common, "GDATA * " #name ":");                           \
        for (iter = irlist_get_head(&gdata.name); iter;                        \
             iter = irlist_get_next(iter)) {
...
void dumpgdata(void) {
    ...
    ioutput(
        gdata_common,
        "GDATA * connectionmethod: how=%d host=%s port=%d passwd=%s vhost=%s",
    ...
    ioutput(gdata_common, "  : when=%ld hostmask=%s message=%s",
            (long)iter->when, gdata_string(iter->hostmask),
            gdata_string(iter->message));
    gdata_irlist_iter_end;


    gdata_irlist_iter_start(dccchats, dccchat_t);
    ioutput(gdata_common, "  : status=%d fd=%d", iter->status, iter->fd);
    ioutput(gdata_common, "  : lastcontact=%ld connecttime=%ld",
            (long)iter->lastcontact, (long)iter->connecttime);
    ioutput(gdata_common,
            "  : localport=%d remoteport=%d localip=0x%.8lX remoteip=0x%.8lX",
            iter->localport, iter->remoteport, iter->localip, iter->remoteip);
    ...
    ioutput(gdata_common, "  : hostmask=%s flags=%d bucket=%ld lastcontact=%ld",
            iter->hostmask, iter->flags, iter->bucket, (long)iter->lastcontact);
    gdata_irlist_iter_end;
...
#ifdef HAVE_MMAP
    {
        ...
        for (iter2 = irlist_get_head(&iter->mmaps); iter2;
    ...
    gdata_iter_print_string(nick);
    gdata_iter_print_string(hostname);
    ioutput(gdata_common, "  : xpack=0x%.8lX queuedtime=%ld",
            (unsigned long)iter->xpack, (long)iter->queuedtime);
    gdata_irlist_iter_end;
    ...
    ioutput(gdata_common,
            ...
            iter->listenport, iter->remoteport, iter->localip, iter->remoteip);
#ifdef HAVE_MMAP
    ...
    ioutput(gdata_common,
            ...
            iter->nomin, iter->nomax, iter->reminded, iter->overlimit,
            iter->tr_status);
    gdata_irlist_iter_end;
...
void clearmemberlist(channel_t* c) {
    /* clear members list */
    if (gdata.debug > 2) {
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_NO_COLOR, "clearing %s", c->name);
    }
...
int isinmemberlist(const char* nick) {
    ...
    if (gdata.debug > 2) {
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_NO_COLOR, "checking for %s",
                nick);
    }
...
void addtomemberlist(channel_t* c, const char* nick) {
    ...
    if (gdata.debug > 2) {
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_NO_COLOR, "adding %s to %s", nick,
                c->name);
    }
...
void removefrommemberlist(channel_t* c, const char* nick) {
    ...
    if (gdata.debug > 2) {
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_NO_COLOR, "removing %s from %s",
                nick, c->name);
    }
...
void changeinmemberlist_mode(channel_t* c, const char* nick, char mode,
    ...
    if (gdata.debug > 2) {
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_NO_COLOR,
                "%s prefix %c on %s in %s", add ? "adding" : "removing", mode,
                nick, c->name);
    }
...
void changeinmemberlist_nick(channel_t* c, const char* oldnick,
    ...
    if (oldmember) {
        memcpy(newmember->prefixes, oldmember->prefixes,
               sizeof(oldmember->prefixes));
...
void set_loginname(void) {
    ...
    if (p == NULL || p->pw_name == NULL) {
        ...
        {
            outerror(OUTERROR_TYPE_WARN_LOUD,
                     "Couldn't Get username, specify loginname in config file");
...
void* irlist_add2(irlist_t* list, unsigned int size, const char* src_function,
    ...
    iitem = mymalloc2(sizeof(irlist_item_t) + size, 1, src_function, src_file,
                      src_line);
...
int ir_bind_listen_socket(int fd, struct sockaddr_in* sa) {
    ...
    if (gdata.debug > 0) {
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_YELLOW, "listen got port %d",
                ntohs(sa->sin_port));
    }
    ...
    while (cur < buffer_len) {
        ...
        if (segment->end == IR_BOUTPUT_SEGMENT_SIZE) {
            ...
            if ((bout->flags & BOUTPUT_NO_LIMIT) ||
                ...
                segment =
                    irlist_add(&bout->segments, sizeof(ir_boutput_segment_t));
            } else {
        ...
        len = min2(IR_BOUTPUT_SEGMENT_SIZE - segment->end, buffer_len - cur);

        if (bout->md5sum) {
            MD5_Update(bout->md5sum, buffer_c + cur, len);
        }
...
int ir_boutput_attempt_flush(ir_boutput_t* bout) {
    ...
    while ((segment = irlist_get_head(&bout->segments))) {
        ...
        retval = write(bout->fd, segment->buffer + segment->begin,
