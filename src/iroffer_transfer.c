...
void t_setuplisten(transfer* const t) {
    ...
    if (gdata.tcprangestart) {
        tempc = 1;
        setsockopt(t->listensocket, SOL_SOCKET, SO_REUSEADDR, &tempc,
                   sizeof(int));
    }
...
void t_establishcon(transfer* const t) {
    ...
    if (t->xpack->file_fd == FD_UNUSED) {
        ...
        if (t->xpack->file_fd < 0) {
            ...
            outerror(OUTERROR_TYPE_WARN_LOUD,
                     "Cant Access Offered File '%s': %s", t->xpack->file,
                     strerror(errno));
    ...
    if (gdata.debug > 0)
        ioutput(CALLTYPE_MULTI_FIRST, OUT_S, COLOR_YELLOW, "SO_SNDLOWAT ");
    getsockopt(t->clientsocket, SOL_SOCKET, 0x1003, &tempc, &tempi);
    if (gdata.debug > 0)
        ioutput(CALLTYPE_MULTI_MIDDLE, OUT_S, COLOR_YELLOW, " %i", tempc);
    ...
    ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_YELLOW,
            ...
            "%ld.%ld.%ld.%ld:%d)",
            t->id, t->nick, t->remoteip >> 24, (t->remoteip >> 16) & 0xFF,
            (t->remoteip >> 8) & 0xFF, t->remoteip & 0xFF, t->remoteport,
            t->localip >> 24, (t->localip >> 16) & 0xFF,
            (t->localip >> 8) & 0xFF, t->localip & 0xFF, t->listenport);
}
...
void t_transfersome(transfer* const t) {
    ...
    do {
        ...
        case TRANSFERMETHOD_LINUX_SENDFILE:
            ...
            howmuch =
                sendfile(t->clientsocket, t->xpack->file_fd, &offset, attempt);

            if (howmuch < 0 && errno == ENOSYS) {
                /* sendfile doesn't work on this system, fall back */
                outerror(OUTERROR_TYPE_WARN,
                         "linux-sendfile transfer method does not work on this "
                         "system, falling back to next available method");
        ...
        case TRANSFERMETHOD_FREEBSD_SENDFILE:
            ...
            j = sendfile(t->xpack->file_fd, t->clientsocket, offset, attempt,
                         &sendfile_header, &offset, 0);
        ...
        case TRANSFERMETHOD_READ_WRITE:
            ...
            if (t->xpack->file_fd_location != t->bytessent) {
                ...
                if (offset != t->bytessent) {
                    outerror(OUTERROR_TYPE_WARN,
                             "Can't seek location in file '%s': %s",
            ...
            if (howmuch < 0 && errno != EAGAIN) {
                outerror(OUTERROR_TYPE_WARN,
                         "Can't read data from file '%s': %s", t->xpack->file,
                         strerror(errno));
        ...
        case TRANSFERMETHOD_MMAP:
            ...
            if (!t->mmap_info || (t->bytessent >= (t->mmap_info->mmap_offset +
                                                   t->mmap_info->mmap_size))) {
                int callval_i;
                mmap_info_t* mm;
                ...
                if (t->mmap_info) {
                    ...
                    if (!t->mmap_info->ref_count) {
                        callval_i = munmap(t->mmap_info->mmap_ptr,
                                           t->mmap_info->mmap_size);
                        if (callval_i < 0) {
                            outerror(OUTERROR_TYPE_WARN,
                                     "Couldn't munmap(): %s", strerror(errno));
                        }
                ...
                for (mm = irlist_get_head(&t->xpack->mmaps); mm;
                     mm = irlist_get_next(mm)) {
                    if (mm->mmap_offset ==
                        (t->bytessent & ~(IR_MMAP_SIZE - 1))) {
                        t->mmap_info = mm;
                        t->mmap_info->ref_count++;
                        break;
                    }
                ...
                if (!t->mmap_info) {
                    ...
                    mm->mmap_ptr =
                        mmap(NULL, mm->mmap_size, PROT_READ, MAP_SHARED,
                             t->xpack->file_fd, mm->mmap_offset);
                    if ((mm->mmap_ptr == (unsigned char*)MAP_FAILED) ||
                        (!mm->mmap_ptr)) {
                        ...
                        if (errno == ENOMEM) {
                            ...
                            outerror(OUTERROR_TYPE_WARN,
                                     "mmap transfer method does not work on "
                                     "this system, falling back to next "
                                     "available method");
                            gdata.transfermethod++;
                        } else {
            ...
            dataptr = t->mmap_info->mmap_ptr + t->bytessent -
                      t->mmap_info->mmap_offset;

            if ((t->bytessent + attempt) >
                (t->mmap_info->mmap_offset + t->mmap_info->mmap_size)) {
                howmuch =
                    (t->mmap_info->mmap_offset + t->mmap_info->mmap_size) -
                    t->bytessent;
            } else {
    ...
    if (t->bytessent >= t->xpack->st_size) {
        ...
        if (t->mmap_info) {
            ...
            if (!t->mmap_info->ref_count) {
                int callval_i;
                callval_i =
                    munmap(t->mmap_info->mmap_ptr, t->mmap_info->mmap_size);
                if (callval_i < 0) {
...
void t_readjunk(transfer* const t) {
    ...
    if (gdata.debug > 4) {
        ...
        for (j = 0; j < i; j++) {
            ioutput(CALLTYPE_MULTI_MIDDLE, OUT_S, COLOR_BLUE, "%2.2X ",
                    gdata.sendbuff[j]);
        }
...
void t_flushed(transfer* const t) {
    ...
    if (timetookms > (60 * 60 * 1000)) {
        snprintf(tempstr + strlen(tempstr), maxtextlength - strlen(tempstr) - 1,
                 " %" PRIu64 "u hr", (uint64_t)timetookms / 60 / 60 / 1000);
    }

    if ((timetookms % (60 * 60 * 1000)) > (60 * 1000)) {
        snprintf(tempstr + strlen(tempstr), maxtextlength - strlen(tempstr) - 1,
                 " %" PRIu64 "u min",
                 ((uint64_t)timetookms % (60 * 60 * 1000)) / 60 / 1000);
    }
    ...
    if (((float)(t->xpack->st_size - t->startresume)) / 1024.0 /
            ((float)timetookms / 1000.0) >
        gdata.record) {
        gdata.record = ((float)(t->xpack->st_size - t->startresume)) / 1024.0 /
                       ((float)timetookms / 1000.0);
    }
...
void t_closeconn(transfer* const t, const char* msg, int errno1) {
    ...
    if (errno1) {
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_YELLOW,
                "XDCC [%02i:%s]: Connection closed: %s (%s)", t->id, t->nick,
                msg, strerror(errno1));
    } else {
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_YELLOW,
                "XDCC [%02i:%s]: Connection closed: %s", t->id, t->nick, msg);
    }
    ...
    if (errno1) {
        notice(t->nick, "** Closing Connection: %s (%s)", msg,
               strerror(errno1));
    } else {
...
void t_remind(transfer* const t) {
    ...
    if (gdata.serverstatus == SERVERSTATUS_CONNECTED) {
        notice(t->nick,
               "** You have a DCC pending, Set your client to receive the "
               "transfer. "
    ...
    if (gdata.punishslowusers) {
        ...
        while (pq) {
            if (strcasecmp(pq->nick, t->nick) == 0) {
                notice(pq->nick,
                       "** Removed From Queue: You are being punished for your "
                       "slowness");
                mydelete(pq->nick);
        ...
        ignore->bucket =
            (gdata.punishslowusers * 60) / gdata.autoignore_threshold;

        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_NO_COLOR,
                "Punish-ignore activated for %s (%s) %d minutes", t->nick,
                ignore->hostmask, gdata.punishslowusers);

        notice(t->nick, "Punish-ignore activated for %s (%s) %d minutes",
               t->nick, ignore->hostmask, gdata.punishslowusers);
