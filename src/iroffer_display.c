...
void initscreen(int startup) {
    ...
    if (tcgetattr(fileno(stdin), &tio) < 0) {
        outerror(OUTERROR_TYPE_CRASH, "can't get terminal settings: %s",
                 strerror(errno));
    }
    ...
    if (tcsetattr(fileno(stdin), TCSANOW, &tio) < 0) {
        outerror(OUTERROR_TYPE_CRASH, "can't set terminal settings: %s",
                 strerror(errno));
    }
    ...
    tostdout("\x1b[%d;1H[ %-*s ]", gdata.termlines - 1, gdata.termcols - 4,
             tempstr);
    mydelete(tempstr);
...
void drawbot(void) {
    ...
    /* draw bottom line */
    tostdout("\x1b[%d;1H[ iroffer (%s) > %-*s]", gdata.termlines,
    ...
    tostdout("\x1b[%d;%dH", gdata.termlines,
             ((gdata.curcol > maxlen) ? maxlen : gdata.curcol) +
                 (gdata.user_nick ? 16 + (int)strlen(gdata.user_nick) : 16));
...
void parseconsole(void) {
    ...
    if (is_fd_readable(fileno(stdin))) {
        ...
        if (length < 1) {
            outerror(OUTERROR_TYPE_CRASH, "read from stdin failed: %s",
        ...
        for (i = 0; i < length; i++) {
            if (console_escape_seq[0] != '\0') {
                        ...
                        if (console_escape_seq[1] == '[') {
                            if (console_escape_seq[esc_len - 1] == 'A') {
                                ...
                                while (count--) {
                                    ...
                                    gdata.console_history_offset = min2(
                                        gdata.console_history_offset,
                                        irlist_size(&gdata.console_history));
                                }
                                ...
                                gdata.curcol =
                                    min2(linelength, gdata.curcol + count);
                ...
                if (irlist_size(&gdata.console_history) > MAX_HISTORY_SIZE) {
                    irlist_delete(&gdata.console_history,
                                  irlist_get_head(&gdata.console_history));
                }
                ...
                if (linelength < (INPUT_BUFFER_LENGTH - 2)) {
                    for (j = linelength; j > gdata.curcol; j--) {
                        gdata.console_input_line[j] =
                            gdata.console_input_line[j - 1];
                    }
                ...
                while (gdata.curcol &&
                       (gdata.console_input_line[gdata.curcol - 1] == ' ')) {
                    for (j = gdata.curcol - 1; j < linelength; j++) {
                        gdata.console_input_line[j] =
                            gdata.console_input_line[j + 1];
                    }
