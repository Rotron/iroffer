...
static const config_parse_bool_t config_parse_bool[] = {
    ...
    {"respondtochannelxdcc", &gdata.respondtochannelxdcc,
     &gdata.respondtochannelxdcc},
    {"respondtochannellist", &gdata.respondtochannellist,
     &gdata.respondtochannellist},
...
static const config_parse_int_t config_parse_int[] = {
    ...
    {"smallfilebypass", &gdata.smallfilebypass, &gdata.smallfilebypass, 0,
     1024 * 1024, 1024},
    {"tcprangestart", &gdata.tcprangestart, &gdata.tcprangestart, 1024, 65000,
     1},
    {"overallmaxspeed", &gdata.overallmaxspeed, &gdata.overallmaxspeed, 0,
     1000000, 4},
    {"overallmaxspeeddayspeed", &gdata.overallmaxspeeddayspeed,
     &gdata.overallmaxspeeddayspeed, 0, 1000000, 4},
    {"queuesize", &gdata.queuesize, &gdata.queuesize, 0, 1000000, 1},
    {"lowbdwth", &gdata.lowbdwth, &gdata.lowbdwth, 0, 1000000, 1},
    {"maxqueueditemsperperson", &gdata.maxqueueditemsperperson,
     &gdata.maxqueueditemsperperson, 1, 1000000, 1},
    {"maxtransfersperperson", &gdata.maxtransfersperperson,
     &gdata.maxtransfersperperson, 1, 1000000, 1},
    {"punishslowusers", &gdata.punishslowusers, &gdata.punishslowusers, 0,
     1000000, 1},
    {"autoignore_threshold", &gdata.autoignore_threshold,
     &gdata.autoignore_threshold, 10, 600, 1},
};
...
static const config_parse_str_t config_parse_str[] = {
    ...
    {"restrictprivlistmsg", &gdata.restrictprivlistmsg,
...
void getconfig_set(const char* line, int rehash) {
    ...
    for (i = 0; i < (sizeof(config_parse_bool) / sizeof(config_parse_bool_t));
         i++) {
        if (!strcmp(config_parse_bool[i].name, type)) {
            if (!strcmp("no", var)) {
                ...
                outerror(OUTERROR_TYPE_WARN,
                         "ignored '%s' because it has unknown args: '%s'", type,
                         var);
            }
            mydelete(var);
            goto done;
    ...
    for (i = 0; i < (sizeof(config_parse_int) / sizeof(config_parse_int_t));
         i++) {
        if (!strcmp(config_parse_int[i].name, type)) {
            ...
            if (var[0] == 0) {
                outerror(OUTERROR_TYPE_WARN,
                         "ignored '%s' because it has no args.", type);
            } else if (endptr[0] != 0) {
                outerror(OUTERROR_TYPE_WARN,
                         "ignored '%s' because it has invalid args: '%s'", type,
                         var);
            } else {
                if ((rawval < config_parse_int[i].min) ||
                    (rawval > config_parse_int[i].max)) {
                    outerror(OUTERROR_TYPE_WARN, "'%s': %d is out-of-range",
                             type, rawval);
                }
                rawval = between(config_parse_int[i].min, rawval,
                                 config_parse_int[i].max);
    ...
    for (i = 0; i < (sizeof(config_parse_str) / sizeof(config_parse_str_t));
         i++) {
    ...
    if (!strcmp(type, "server")) {
        ...
        if (cptr->plisttime && (cptr->plistoffset >= cptr->plisttime)) {
            outerror(
                OUTERROR_TYPE_WARN,
                "plistoffset must be less than plist time, ignoring offset");
            cptr->plistoffset = 0;
        }
        ...
        if ((i < 0) || (!strchr(var, '@'))) {
            outerror(OUTERROR_TYPE_WARN,
                     "adminhost '%s' ignored because it's way too vague", var);
        } else {
            char* varr;
        ...
        if (gdata.slotsmax != atoi(var)) {
            outerror(OUTERROR_TYPE_WARN,
                     "unable to have slotsmax of %d, using %d instead",
                     atoi(var), gdata.slotsmax);
        }
        mydelete(var);
        ...
        if (a && b && c) {
            ...
            gdata.autosend.message =
                mymalloc(strlen(var) - 2 - strlen(a) - strlen(b) + 1);
        ...
        if (!gdata.periodicmsg_nick || !tnum || !gdata.periodicmsg_msg) {
            outerror(OUTERROR_TYPE_WARN_LOUD,
                     "Syntax Error In periodicmsg, Ignoring");
            mydelete(gdata.periodicmsg_nick);
            mydelete(gdata.periodicmsg_msg);
            mydelete(var);
        } else {
        ...
        if (sscanf(var, "%lu.%lu.%lu.%lu", &ipparts[0], &ipparts[1],
                   &ipparts[2], &ipparts[3]) < 4) {
            ...
            if (((gdata.ourip & 0xFF000000UL) == 0x0A000000UL) ||
                ...
                outerror(OUTERROR_TYPE_WARN_LOUD,
                         "usenatip of %lu.%lu.%lu.%lu looks wrong, this is "
                         "probably not what you want to do",
                         (gdata.ourip >> 24) & 0xFF, (gdata.ourip >> 16) & 0xFF,
                         (gdata.ourip >> 8) & 0xFF, (gdata.ourip) & 0xFF);
            }
        }
        mydelete(var);
        ...
        if (sscanf(var, "%lu.%lu.%lu.%lu", &ipparts[0], &ipparts[1],
            ...
            if (rehash) {
                gdata.r_local_vhost = (ipparts[0] << 24) | (ipparts[1] << 16) |
                                      (ipparts[2] << 8) | ipparts[3];
            } else {
                gdata.local_vhost = (ipparts[0] << 24) | (ipparts[1] << 16) |
                                    (ipparts[2] << 8) | ipparts[3];
            }
        }
        mydelete(var);
        ...
        if (thow && !strcmp(thow, "direct")) {
            ...
            gdata.connectionmethod.how = how_direct;
            outerror(OUTERROR_TYPE_WARN_LOUD,
                     "Invalid connectionmethod in config file, defaulting to "
                     "direct");
        }
        ...
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Ignored invalid line in config file: %s", type);
        mydelete(var);
    }
...
static int connectirc(server_t* tserver) {
    ...
    case how_bnc:
        if (gdata.connectionmethod.vhost) {
            snprintf(tempstr, maxtextlength - 1, " (bnc at %s:%i with %s)",
                     gdata.connectionmethod.host, gdata.connectionmethod.port,
                     gdata.connectionmethod.vhost);
        } else {
            snprintf(tempstr, maxtextlength - 1, " (bnc at %s:%i)",
                     gdata.connectionmethod.host, gdata.connectionmethod.port);
        }
    ...
    case how_wingate:
        snprintf(tempstr, maxtextlength - 1, " (wingate at %s:%i)",
                 gdata.connectionmethod.host, gdata.connectionmethod.port);
        gdata.serv_resolv.to_ip = gdata.connectionmethod.host;
        gdata.serv_resolv.to_port = gdata.connectionmethod.port;
        break;
    ...
    case how_custom:
        snprintf(tempstr, maxtextlength - 1, " (custom at %s:%i)",
                 gdata.connectionmethod.host, gdata.connectionmethod.port);
        gdata.serv_resolv.to_ip = gdata.connectionmethod.host;
        gdata.serv_resolv.to_port = gdata.connectionmethod.port;
        break;
    ...
    if (gdata.local_vhost) {
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_NO_COLOR,
                ...
                gdata.local_vhost >> 24, (gdata.local_vhost >> 16) & 0xFF,
                (gdata.local_vhost >> 8) & 0xFF, gdata.local_vhost & 0xFF,
                tempstr);
    } else {
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_NO_COLOR,
                "Attempting Connection to %s:%u%s", gdata.curserver.hostname,
                gdata.curserver.port, tempstr);
    }
    ...
    if (callval < 0) {
        ...
        callval = write(gdata.serv_resolv.sp_fd[1], remotehost->h_addr_list[0],
...
int connectirc2(struct in_addr* remote) {
    ...
    if (gdata.debug > 0) {
        ...
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_YELLOW,
                "Connecting to %lu.%lu.%lu.%lu:%d", (to_ip >> 24) & 0xFF,
                (to_ip >> 16) & 0xFF, (to_ip >> 8) & 0xFF, (to_ip)&0xFF,
    ...
    if (gdata.local_vhost) {
        ...
        if (bind(gdata.ircserver, (struct sockaddr*)&localaddr,
                 sizeof(localaddr)) < 0) {
    ...
    if (gdata.debug > 0) {
        ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_YELLOW, "ircserver socket = %d",
                gdata.ircserver);
    }
...
void initirc(void) {
    ...
    if (gdata.connectionmethod.how == how_wingate) {
        writeserver(WRITESERVER_NOW, "%s %u", gdata.curserver.hostname,
    ...
    writeserver(WRITESERVER_NOW, "USER %s 32 . :%s", gdata.loginname,
                gdata.user_realname);
    ...
    if (gdata.connectionmethod.how == how_bnc) {
        ...
        writeserver(WRITESERVER_NOW, "CONN %s %d", gdata.curserver.hostname,
                    gdata.curserver.port);
    }
...
void vwriteserver(writeserver_type_e type, const char* format, va_list ap) {
    ...
    if ((len < 0) || (len >= maxtextlength)) {
        outerror(OUTERROR_TYPE_WARN,
                 "WRITESERVER: Output too large, ignoring!");
        mydelete(msg);
        return;
    }
    ...
    if ((type == WRITESERVER_NOW) &&
        ...
        if (type == WRITESERVER_FAST) {
            if (gdata.debug > 0) {
                ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_MAGENTA, "<QUEF<: %s",
                        msg);
            }
            ...
            if (irlist_size(&gdata.serverq_fast) < MAXSENDQ) {
                ...
                outerror(
                    OUTERROR_TYPE_WARN,
                    "Server queue is very large. Dropping additional output.");
            }
        } else if (type == WRITESERVER_NORMAL) {
            if (gdata.debug > 0) {
                ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_MAGENTA, "<QUEN<: %s",
                        msg);
            }
            ...
            if (gdata.debug > 0) {
                ioutput(CALLTYPE_NORMAL, OUT_S, COLOR_MAGENTA, "<QUES<: %s",
                        msg);
            }
...
void sendserver(void) {
    ...
    if ((irlist_size(&gdata.serverq_fast) == 0) &&
        ...
        (irlist_size(&gdata.serverq_slow) == 0) && gdata.exiting &&
        !gdata.recentsent) {
        ...
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_D, COLOR_NO_COLOR,
                "Connection to %s (%s) Closed", gdata.curserver.hostname,
...
void xdccsavetext(void) {
    ...
    if ((callval < 0) && (errno != ENOENT)) {
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Cant Remove Old XDCC List File '%s': %s", xdcclistfile_bkup,
                 strerror(errno));
        /* ignore, continue */
    }
    ...
    if ((callval < 0) && (errno != ENOENT)) {
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Cant Backup Old XDCC List File '%s' -> '%s': %s",
    ...
    if (callval < 0) {
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Cant Save New XDCC List File '%s': %s", gdata.xdcclistfile,
                 strerror(errno));
        /* ignore, continue */
    }
...
void shutdowniroffer(void) {
    ...
    if (gdata.exiting || gdata.serverstatus != SERVERSTATUS_CONNECTED) {
        ...
        for (chat = irlist_get_head(&gdata.dccchats); chat;
            ...
            writedccchat(chat, 0,
                         "iroffer exited (shutdown), Closing DCC Chat\n");
    ...
    while (tr) {
        notice(tr->nick,
               "** Shutting Down. Closing Connection. (Resume Supported)");
        ...
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_D, COLOR_YELLOW,
                "XDCC Transfer to %s Closed", tr->nick);
    ...
    while (ul) {
        notice(
            ul->nick,
            "** Shutting Down. Closing Upload Connection. (Resume Supported)");
    ...
    if (gdata.serverstatus == SERVERSTATUS_CONNECTED) {
        ...
        writeserver(WRITESERVER_NORMAL,
                    ...
                    gdata.hideos ? "" : " - ",
                    gdata.hideos ? "" : gdata.osstring, tempstr2);
        mydelete(tempstr2);
    }
...
char* getstatusline(char* str, int len) {
    ...
    i = snprintf(str, len,
                 ...
                 irlist_size(&gdata.mainqueue), gdata.queuesize, gdata.record,
                 srvq, (xdccsent / 1024),
...
char* getstatuslinenums(char* str, int len) {
    ...
    i = snprintf(str, len,
                 ...
                 scount / 1024 / 1024, irlist_size(&gdata.trans),
                 gdata.slotsmax, irlist_size(&gdata.mainqueue), gdata.queuesize,
                 0, 0, gdata.record, srvq, (xdccsent / 1024),
                 ((float)xdccsent) / XDCC_SENT_SIZE / 1024.0, gdata.sentrecord);
    if ((i < 0) || (i >= len)) {
        str[0] = '\0';
    }
...
void sendxdlqueue(void) {
    ...
    if (gdata.nolisting > gdata.curtime) {
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_D, COLOR_YELLOW,
                "Not Sending XDCC LIST to: %s (nolist set)", tempstr);

        notice(tempstr,
               "The Owner Has Requested That No Lists Be Sent In The Next %li "
               "Minute%s",
               1 + (gdata.nolisting - gdata.curtime) / 60,
               ((1 + (gdata.nolisting - gdata.curtime) / 60) != 1 ? "s" : ""));
    } else {
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_D, COLOR_YELLOW,
                "Sending XDCC LIST to: %s", tempstr);
...
int isthisforme(const char* dest, char* msg1) {
    if (!msg1 || !dest) {
        outerror(OUTERROR_TYPE_WARN_LOUD, "isthisforme() got NULL value");
        return 1;
    }
    ...
    if (!strcmp(msg1, "\1CLIENTINFO") || !strcmp(msg1, "\1CLIENTINFO\1") ||
...
void reinit_config_vars(void) {
    ...
    for (rh = irlist_get_head(&gdata.autoignore_exclude); rh;
         rh = irlist_delete(&gdata.autoignore_exclude, rh)) {
        regfree(rh);
    }
    for (rh = irlist_get_head(&gdata.adminhost); rh;
         rh = irlist_delete(&gdata.adminhost, rh)) {
        regfree(rh);
    }
    for (rh = irlist_get_head(&gdata.uploadhost); rh;
         rh = irlist_delete(&gdata.uploadhost, rh)) {
        regfree(rh);
    }
    for (rh = irlist_get_head(&gdata.downloadhost); rh;
         rh = irlist_delete(&gdata.downloadhost, rh)) {
        regfree(rh);
    }
    {
        server_t* ss;
        for (ss = irlist_get_head(&gdata.servers); ss;
...
void initvars(void) {
    ...
    /* 4GB max size */
    /* NOTE: DCC protocol can't handle more than 32bit file size, 4GB is it,
     * sorry */
...
void startupiroffer(void) {
    ...
    if (gdata.runasuser) {
        ...
        if (getgrouplist(pw_ent->pw_name, pw_ent->pw_gid, groups, &ngroups) <
            0) {
            mydelete(groups);
            groups = mycalloc(ngroups * sizeof(gid_t));
            if (getgrouplist(pw_ent->pw_name, pw_ent->pw_gid, groups,
                             &ngroups) < 0) {
                outerror(OUTERROR_TYPE_CRASH, "Can't lookup group list: %s",
                         strerror(errno));
            }
        }
#endif
    ...
    if (gdata.runasuser) {
        ...
        if (setgid(runasgid) < 0) {
            outerror(OUTERROR_TYPE_CRASH, "Can't change group: %s",
                     strerror(errno));
        }
        if (setgroups(ngroups, groups) < 0) {
            outerror(OUTERROR_TYPE_CRASH, "Can't set group list: %s",
                     strerror(errno));
        }
        if (setuid(runasuid) < 0) {
            outerror(OUTERROR_TYPE_CRASH, "Can't change to user: %s",
                     strerror(errno));
        }
        mydelete(groups);
    }
    ...
    if (callval >= 0) {
        ...
        outerror(OUTERROR_TYPE_CRASH, "Unable to read fd limit: %s",
                 strerror(errno));
    }
    ...
    if (gdata.adjustcore) {
        ...
        if (callval >= 0) {
            ...
            if (rlim.rlim_cur == 0) {
                outerror(OUTERROR_TYPE_WARN,
                         "Unable to set core limit: hard limit is 0");
            }
            ...
            outerror(OUTERROR_TYPE_WARN, "Unable to read core limit: %s",
                     strerror(errno));
        }
    }
    ...
    if (gdata.background) {
        ...
        outerror(OUTERROR_TYPE_WARN_LOUD, "Cant set console non-blocking!: %s",
                 strerror(errno));
    } else {
...
void isrotatelog(void) {
    ...
    if (!gdata.logrotate || !gdata.logfile ||
    ...
    sprintf(newname, "%s.%04i-%02i-%02i", gdata.logfile, lthen->tm_year + 1900,
            lthen->tm_mon + 1, lthen->tm_mday);

    mylog(CALLTYPE_NORMAL, "Rotating Log to '%s'", newname);
    call_val = link(gdata.logfile, newname);

    if (call_val < 0) {
        outerror(OUTERROR_TYPE_WARN, "Unable to rename log '%s' -> '%s': %s",
                 gdata.logfile, newname, strerror(errno));
    } else {
        ioutput(CALLTYPE_NORMAL, OUT_S | OUT_L | OUT_D, COLOR_NO_COLOR,
        ...
        if (call_val < 0) {
            outerror(OUTERROR_TYPE_WARN, "Unable to remove old log '%s' : %s",
...
void createpassword(void) {
    ...
    printf("\niroffer v" VERSIONLONG
           ...
           "You should place the output this program generates in your config "
           "file.\n"
...
#else
    printf(
        "iroffer was compiled without encrypted password support.  You do not "
        "need to ecrypt your password.\n");
#endif
}
...
void notifyqueued(void) {
    ...
    if (!gdata.exiting && irlist_size(&gdata.mainqueue)) {
        if (gdata.lowbdwth) {
            ...
            ioutput(
                ...
                ((float)xdccsent) / XDCC_SENT_SIZE / 1024.0, gdata.lowbdwth);
        } else {
            ioutput(CALLTYPE_NORMAL, OUT_S | OUT_D, COLOR_YELLOW,
    ...
    while (pq) {
        ...
        notice_slow(pq->nick,
                    "Queued %lih%lim for \"%s\", in position %i of %i. "
                    "%lih%lim or %s remaining.",
                    (long)(gdata.curtime - pq->queuedtime) / 60 / 60,
                    (long)((gdata.curtime - pq->queuedtime) / 60) % 60,
                    pq->xpack->desc, i, irlist_size(&gdata.mainqueue),
                    lastrtime / 60 / 60, (lastrtime / 60) % 60,
...
void notifybandwidth(void) {
    ...
    if ((xdccsent * 10) > (gdata.maxb * 30 * 9)) {
        ...
        while (tr) {
            notice_slow(tr->nick,
                        "%s bandwidth limit: %2.1f of %2.1fKB/sec used. Your "
                        "share: %2.1fKB/sec.",
                        (gdata.user_nick ? gdata.user_nick : "??"),
                        ((float)xdccsent) / XDCC_SENT_SIZE,
                        ((float)gdata.maxb) / 4.0, tr->lastspeed);
            tr = irlist_get_next(tr);
        }
    }
}
...
void notifybandwidthtrans(void) {
    ...
    while (tr) {
        if (!tr->nomax && (tr->xpack->maxspeed > 0) &&
            ...
            /* send if over 90% */
            notice_slow(tr->nick,
                        "Pack bandwidth limit: %2.1f of %2.1fKB/sec used.",
                        tr->lastspeed, tr->xpack->maxspeed);
        }
        tr = irlist_get_next(tr);
    }
}
...
void look_for_file_changes(xdcc* xpack) {
    ...
    if (stat(xpack->file, &st) < 0) {
        outerror(OUTERROR_TYPE_WARN, "File '%s' can no longer be accessed: %s",
                 xpack->file, strerror(errno));
        return;
    }

    if (st.st_size == 0) {
        outerror(OUTERROR_TYPE_WARN, "File '%s' has size of 0 bytes",
                 xpack->file);
    }

    if ((st.st_size > gdata.max_file_size) || (st.st_size < 0)) {
        outerror(OUTERROR_TYPE_WARN, "File '%s' is too large", xpack->file);
    }
...
void reverify_restrictsend(void) {
    ...
    if (!gdata.restrictsend || (gdata.serverstatus != SERVERSTATUS_CONNECTED)) {
        return;
    }
    ...
    for (tr = irlist_get_head(&gdata.trans); tr; tr = irlist_get_next(tr)) {
        if ((tr->tr_status != TRANSFER_STATUS_DONE) &&
            ...
            } else if ((gdata.curtime - tr->restrictsend_bad) >=
                       RESTRICTSEND_TIMEOUT) {
    ...
    for (pq = irlist_get_head(&gdata.mainqueue); pq;) {
        if (strcmp(pq->hostname, "man") != 0) {
            if (isinmemberlist(pq->nick)) {
                ...
                pq = irlist_get_next(pq);
            } else if ((gdata.curtime - pq->restrictsend_bad) >=
                       RESTRICTSEND_TIMEOUT) {
                notice(pq->nick,
                       "** Removed From Queue: You are no longer on a known "
                       "channel");
                mydelete(pq->nick);
