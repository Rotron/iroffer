...
static void getxdccconfig(const char* filename) {
    ...
    filedescriptor = open(filename, O_RDONLY | O_CREAT | ADDED_OPEN_FLAGS,
                          CREAT_PERMISSIONS);
    if (filedescriptor < 0)
        outerror(OUTERROR_TYPE_CRASH, "Cant Access XDCC File '%s': %s",
    ...
    if (getfline(templine1, maxtextlength, filedescriptor, 0) != NULL) {
        ...
        if (ok) {
            outerror(OUTERROR_TYPE_WARN_LOUD,
                     "Invalid First Line, Reverting Values to 0");
        ...
        outerror(OUTERROR_TYPE_WARN_LOUD,
                 "Empty XDCC File, Starting With No Packs Offered");
    ...
    while (getfline(templine1, maxtextlength, filedescriptor, 1) != NULL) {
        if (getfline(templine1, maxtextlength, filedescriptor, 0) == NULL)
            ok++;
        if (getfline(templine2, maxtextlength, filedescriptor, 0) == NULL)
            ok++;
        if (getfline(templine3, maxtextlength, filedescriptor, 0) == NULL)
            ok++;
        if (getfline(templine4, maxtextlength, filedescriptor, 0) == NULL)
            ok++;
        if (getfline(templine5, maxtextlength, filedescriptor, 0) == NULL)
            ok++;
        if (getfline(templine6, maxtextlength, filedescriptor, 0) == NULL)
            ok++;

        if (ok)
            outerror(OUTERROR_TYPE_CRASH,
                     "XDCC file syntax error (missing/extra line)");
        ...
        if ((strlen(templine1) < 8) || strncmp(templine1 + 3, "file ", 5))
            ok++;
        if ((strlen(templine2) < 8) || strncmp(templine2 + 3, "desc ", 5))
            ok++;
        if ((strlen(templine3) < 8) || strncmp(templine3 + 3, "note ", 5))
            ok++;
        if ((strlen(templine4) < 8) || strncmp(templine4 + 3, "gets ", 5))
            ok++;
        if ((strlen(templine5) < 8) || strncmp(templine5 + 3, "mins ", 5))
            ok++;
        if ((strlen(templine6) < 8) || strncmp(templine6 + 3, "maxs ", 5))
            ok++;

        if (ok)
            outerror(OUTERROR_TYPE_CRASH,
                     "XDCC file syntax error (incorrect order?)");
        ...
        if (xfiledescriptor < 0) {
            ...
            outerror(OUTERROR_TYPE_WARN,
                     "Cant Access Offered File Details '%s': %s", xd->file,
                     strerror(errno));
            memset(&st, 0, sizeof(st));
        }
    ...
    if (!gdata.totalsent) {
        ...
        while (xd) {
            gdata.totalsent += ((unsigned long long)xd->gets) *
                               ((unsigned long long)xd->st_size);
...
int main(int argc, char* argv[]) {
    ...
    if ((optind != argc) || !xdccfile || !gdata.statefile) {
        usage(argv[0]);
    }
